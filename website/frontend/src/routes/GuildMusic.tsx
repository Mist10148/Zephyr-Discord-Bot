...
export function GuildMusic() {
  ...
  if (player.isPending) return <main className="app"><Skeleton variant="now-playing" /><Skeleton variant="rows" count={3} /><Skeleton variant="rows" count={4} /></main>; if (player.error || !player.data) return <main className="app"><LargeTitleHeader title="Music" /><ErrorNote error={player.error} onRetry={() => player.refetch()} /><BackLink to={`/g/${guildId}`}>Back to the server</BackLink></main>
  ...
  return <main className="app"><GuildShell guildId={guildId}><div className="player-live" aria-live="polite">{data.track ? `${data.track.title} ${paused ? 'paused' : 'playing'}` : 'Nothing playing'}</div><LargeTitleHeader title="Music" subtitle="Now playing, queue and playlists for this server." />
    {/* One grid, placed rather than reordered. At narrow widths this is
        `display: contents`, so the DOM order below *is* the phone order and the
        860px arrangement is untouched. At >=1200px the three groups are assigned
        to cells: player and effects left, search and queue right -- which is the
        only way to get that without moving the search field away from the top
        of the phone layout. */}
    ...
    <div className="music-col-player">
