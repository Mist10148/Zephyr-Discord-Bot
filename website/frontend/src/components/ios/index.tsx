...
// `interactive` adds a hover lift for glass cards that are themselves a link/button
// target; plain informational cards stay still.
export function GlassSurface({ children, className = '', interactive = false }: { children: ReactNode; className?: string; interactive?: boolean }) { return <section className={`glass ${interactive ? 'glass-interactive' : ''} ${className}`.trim()}>{children}</section> }
...
export function SegmentedControl({ values, value, onChange }: { values: string[]; value: string; onChange(value: string): void }) { return <div className="segmented" role="group">{values.map(item => <button key={item} className={item === value ? 'active' : ''} aria-pressed={item === value} onClick={() => onChange(item)}>{item}</button>)}</div> }
...
export function Slider({ value, onChange, label }: { value: number; onChange(value: number): void; label?: string }) { return <input className="slider" type="range" value={value} onChange={event => onChange(+event.target.value)} aria-label={label} aria-valuetext={String(value)} /> }
...
export function LargeTitleHeader({ title, subtitle }: { title: string; subtitle?: string }) { return <header className="large-title"><h1>{title}</h1>{subtitle && <p className="subtitle">{subtitle}</p>}</header> }
