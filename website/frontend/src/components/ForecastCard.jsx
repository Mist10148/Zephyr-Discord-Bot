...
export default function ForecastCard({ day, unit }) {
  ...
  const dayTemp = unit === 'C' ? day.day_temp_c : day.day_temp_f;
  const nightTemp = unit === 'C' ? day.night_temp_c : day.night_temp_f;

  return (
    <div
      className={`glass flex-shrink-0 w-64 transition-all duration-300 hover:-translate-y-1 ${
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {getWeatherIconComponent(day.day_desc, 48, day.icon)}
            <div>
              <p className="text-white font-medium">{dayTemp}°</p>
              <p className="text-white/50 text-xs capitalize">{day.day_desc}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-white/60 text-sm">{nightTemp}°</p>
      ...
      {expanded && (
        <div className="px-5 pb-5 pt-0 border-t border-white/10">
          <div className="grid grid-cols-2 gap-3 mt-4">
            <DetailRow icon={<Sun className="w-4 h-4 text-amber-300" />} label="Day" value={`${dayTemp}°${unit}`} />
            <DetailRow icon={<Moon className="w-4 h-4 text-indigo-300" />} label="Night" value={`${nightTemp}°${unit}`} />
